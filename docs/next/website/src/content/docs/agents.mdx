...
On Linux and macOS, wrappers such as VMs, Bubblewrap, `fence`, or sandboxes can hide the real agent process from host process inspection. Set `HERDR_AGENT=<agent>` on the command, for example `HERDR_AGENT=claude fence -- claude`, to tell Herdr which existing agent screen manifest to use. The hint is scoped to that foreground process; avoid exporting it globally from your shell unless every inherited foreground process should be treated as that agent.
